...
function ActionButton({ onClick, title, children, danger = false }) {
  return (
    <Tooltip.Root>
      ...
      <Tooltip.Portal>
        <Tooltip.Content className="bg-zinc-800 text-zinc-100 text-xs px-2 py-1 rounded-md shadow-lg" sideOffset={4}>
          {title}
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right shrink-0">
          <p className="text-xl font-bold text-white tabular-nums">{formatPrice(sub.price)}</p>
          <p className="text-xs text-zinc-500 mt-0.5">{formatInterval(sub.interval_value, sub.interval_unit)}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1 text-sm">
        ...
        {sub.is_trial && sub.trial_end_date && (
          <div className="flex justify-between">
            ...
            <span className={cn('tabular-nums', isExpiringSoon ? 'text-red-400' : 'text-zinc-300')}>
      ...
      <Tooltip.Provider delayDuration={300}>
        <div className="flex items-center justify-end gap-1 -m-1">
          <ActionButton onClick={() => onToggleMute(sub.id)} title={sub.mute_notifs ? 'Unmute notifications' : 'Mute notifications'}>
